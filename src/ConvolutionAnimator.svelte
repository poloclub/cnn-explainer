<script>
  ...
  function startConvolution(stride) {
    counter = 0;
    let outputMappings = generateOutputMappings(stride, output, kernel, padded_input_size, dilation);
    if (stride <= 0) return;
    ...
    interval = setInterval(() => {
      ...
      inputHighlights = compute_input_multiplies_with_weight(animatedH, animatedW, padded_input_size, kernel.length, outputMappings, kernel)
      const inputMatrixSlice = getMatrixSliceFromInputHighlights(image, inputHighlights, kernel);
      testInputMatrixSlice = gridData(inputMatrixSlice);
      const outputMatrixSlice = getMatrixSliceFromOutputHighlights(output, outputHighlights, kernel);
